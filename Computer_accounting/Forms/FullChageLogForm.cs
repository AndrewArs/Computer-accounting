...
{
    public partial class FullChageLogForm : Form
    {
        MySqlConnection conn;

        public FullChageLogForm(MySqlConnection conn)
        {
            InitializeComponent();
        ...
        {
            MySqlCommand cmd = new MySqlCommand("SELECT * FROM computers_log", conn);

            using (MySqlDataReader dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    dataGridView.Rows.Add(dr.GetDateTime(10).ToString("yyyy-MM-dd"),dr.GetValue(1), 
